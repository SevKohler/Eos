...
public class RangeLowConverter extends CDMConverter<Double> {

    public RangeLowConverter() {
        super(RangeLowConverter.class);
    }
    ...
    protected Optional<Double> convertDvValue(DataValue value) {
        if (value.getClass() == DvQuantity.class) {
            ...
            LOG.warn("The data value class: " + value.getClass() + " is not supported for the Measurement range lower mapping or normal range lower is not populated. Nothing is mapped for operator.");
    ...
    private Optional<Double> convertDvQuantity(DvQuantity quantity) {
        if (quantity.getNormalRange() != null ) {
            try{
                return Optional.of(Objects.requireNonNull(quantity.getNormalRange().getLower()).getMagnitude());
    ...
    private Optional<Double> convertDvProportion(DvProportion proportion) {
        if (proportion.getNormalRange() != null) {
            try{
                return Optional.of(Objects.requireNonNull(proportion.getNormalRange().getLower()).getMagnitude());
            }catch (NullPointerException npe){
                return Optional.empty();
            }
        }
    ...
    private Optional<Double> convertDvOrdinal(DvOrdinal ordinal) {
        if (ordinal.getNormalRange() != null) {
            try{
                Long result = Objects.requireNonNull(ordinal.getNormalRange().getLower()).getValue();
